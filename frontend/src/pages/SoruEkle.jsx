...
// --- GELİŞMİŞ GÖRSEL EDİTÖRÜ (WIDTH & HEIGHT & STRETCH) ---
const ResizableImage = ({ src, width, height, align, onUpdate, onDelete }) => {
  const [isResizing, setIsResizing] = useState(false);
  const [resizeMode, setResizeMode] = useState(null); // 'se' (corner), 'e' (width), 's' (height)
  ...
  const handleMouseDown = (e, mode) => {
    ...
    const doDrag = (dragEvent) => {
      ...
      // YÜKSEKLİK AYARI (Piksel bazlı - Sündürme/Stretch)
      if (mode === 'se' || mode === 's') {
        // Eğer sadece SE (Köşe) ise ve height 'auto' ise orantılı kalır.
        // Ama kullanıcı 's' (alt) kullandıysa sündürmeye başlar.
    ...
    >
      <img
        ...
        style={{
          ...
          objectFit: height === 'auto' ? 'contain' : 'fill' // Height varsa sündür (fill)
        }}
        alt="Soru Görseli"
      />
      ...
      <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-white shadow-lg rounded-lg p-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition z-50 pointer-events-none group-hover:pointer-events-auto border border-gray-200">
        <button onClick={() => onUpdate({ align: 'left' })} className={`p-1 rounded ${align === 'left' ? 'bg-blue-100' : 'hover:bg-gray-100'}`} title="Sola Yasla"><Bars3BottomLeftIcon className="w-4 h-4" /></button>
        <button onClick={() => onUpdate({ align: 'center' })} className={`p-1 rounded ${align === 'center' ? 'bg-blue-100' : 'hover:bg-gray-100'}`} title="Ortala"><Bars3Icon className="w-4 h-4" /></button>
        <button onClick={() => onUpdate({ align: 'right' })} className={`p-1 rounded ${align === 'right' ? 'bg-blue-100' : 'hover:bg-gray-100'}`} title="Sağa Yasla"><Bars3BottomRightIcon className="w-4 h-4" /></button>
        <div className="w-[1px] bg-gray-300 mx-1"></div>
        <button onClick={() => onUpdate({ height: 'auto' })} className="p-1 rounded hover:bg-gray-100 text-xs font-bold" title="Oranı Sıfırla (Auto)">Oto</button>
        <button onClick={onDelete} className="p-1 rounded hover:bg-red-100 text-red-500" title="Sil"><TrashIcon className="w-4 h-4" /></button>
      </div>

      {/* HANDLES */}
      {/* 1. Sağ Kenar (Sadece Genişlik - Enine) */}
      <div
        onMouseDown={(e) => handleMouseDown(e, 'e')}
        className="absolute top-0 right-0 bottom-0 w-4 cursor-ew-resize opacity-0 group-hover:opacity-100 hover:bg-blue-500/20 z-10"
      ></div>

      {/* 2. Alt Kenar (Sadece Yükseklik - Boyuna) */}
      <div
        onMouseDown={(e) => handleMouseDown(e, 's')}
        className="absolute bottom-0 left-0 right-0 h-4 cursor-ns-resize opacity-0 group-hover:opacity-100 hover:bg-blue-500/20 z-10"
      ></div>

      {/* 3. Sağ Alt Köşe (Hem En Hem Boy - Orantılı veya Serbest) */}
      <div
        onMouseDown={(e) => handleMouseDown(e, 'se')}
        className="absolute bottom-0 right-0 w-6 h-6 bg-blue-500 cursor-nwse-resize opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-tl-lg shadow-sm z-20"
      >
      ...
      {isResizing && (
        <div className="absolute bottom-2 right-8 bg-black/70 text-white text-xs px-2 py-1 rounded">
          W: {Math.round(width)}% {height !== 'auto' ? `H: ${height}px` : '(Oto)'}
        </div>
      )}
    </div>
  );
...
export default function SoruEkle() {
  ...
  const [widthMode, setWidthMode] = useState('dar'); // 'dar' | 'genis'
  // components: { id, type: 'text'|'image', subtype: 'koku'|'govde'|'secenek', content, width, height, align }
  const [components, setComponents] = useState([
    { id: 'init_govde', type: 'text', subtype: 'govde', content: '', placeholder: 'Soru metnini buraya giriniz...' }
  ]);
  ...
  // --- PARÇA EKLEME FONKSİYONLARI ---

  // 1. Soru Gövdesi (Normal Metin)
  const addGovde = () => {
    setComponents([...components, { id: Date.now(), type: 'text', subtype: 'govde', content: '', placeholder: 'Soru anlatımı/paragraf...' }]);
  };

  // 2. Soru Kökü (Kalın, Highlight)
  const addKoku = () => {
    setComponents([...components, {
      ...
      subtype: 'koku',
      content: '',
      placeholder: 'Soru kökü (Buna göre... hangisidir?)'
    }]);
  };

  // 3. Seçenekler (Otomatik Şablon)
  const addSecenekler = () => {
    // 5 satırlı bir yapı ekleyelim
    const baseId = Date.now();
    const opts = ['A', 'B', 'C', 'D', 'E'].map((opt, idx) => ({
      id: baseId + idx,
      type: 'text',
      subtype: 'secenek',
      content: `<b>${opt})</b> `,
    ...
    try {
      ...
      let htmlContent = components.map(c => {
        ...
        if (c.type === 'image') {
          ...
          else if (c.subtype === 'secenek') style = "margin-bottom: 4px; padding-left: 8px;";
          else style = "margin-bottom: 8px;";

          return `<div class="q-txt q-${c.subtype}" style="${style} clear: both;">${c.content}</div>`;
        }
      }).join('');
    ...
    <div className="min-h-screen bg-[#F3F2F1] pb-32 font-sans">
      ...
      <div className="flex justify-center p-8 overflow-y-auto">
        <div className="flex flex-col gap-4 sticky top-20 h-fit mr-4">
          {/* SOL ARAÇ ÇUBUĞU (SORU PARÇALARI) */}
          <div className="bg-white p-2 rounded shadow border flex flex-col gap-2 w-32">
            ...
            <button onClick={addSecenekler} className="flex items-center gap-2 p-2 hover:bg-green-50 text-green-700 rounded text-sm font-bold border border-transparent hover:border-green-200 transition text-left">
              <QueueListIcon className="w-5 h-5" /> Seçenekler
            </button>
          ...
          <div className="space-y-1 relative" style={{ fontFamily: '"Arial", sans-serif', fontSize: '10pt', lineHeight: '1.4' }}>
            ...
            {components.map((comp, index) => (
              <div key={comp.id} className="relative group/item hover:ring-1 hover:ring-blue-100 rounded px-1 transition">
                {/* Sıralama */}
                <div className="absolute -left-8 top-1 flex flex-col gap-1 opacity-0 group-hover/item:opacity-100 transition z-10 w-6">
